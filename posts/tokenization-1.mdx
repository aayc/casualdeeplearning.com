...
`ing`. BPE can have trouble when there are multiple ways to encode a word, leading
...
Technically SentencePiece is not a tokenization algorithm; it is some extra features on top of common tokenization strategies like BPE. SentencePiece handles problems like "how do we avoid multiple ways to represent the same word" (subword regularization), and "how do we handle whitespaces."
...
Move on to [Level 2: Practical](https://localhost:3000/posts/tokenization-2) to see how tokenizers work in a framework like PyTorch.
